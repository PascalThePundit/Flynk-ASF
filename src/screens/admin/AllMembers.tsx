    ...
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col pb-24 md:pb-8">
      ...
      <div className="p-4 space-y-3">
        {filteredUsers.map(user => (
          <div key={user.uid} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between hover:border-[#D4A843] transition-all">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center font-bold text-gray-400">
                {user.avatarUrl ? (
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <h3 className="font-bold text-[#0A1628] text-sm font-display">{user.name}</h3>
